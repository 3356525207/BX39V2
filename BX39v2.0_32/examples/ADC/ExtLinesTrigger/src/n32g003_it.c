...
/**
*\*\name    ADC_IRQHandler.
*\*\fun     This function handles ADC global interrupts requests.
*\*\return  None
**/
void ADC_IRQHandler(void)
{
    if(ADC_INTFlag_Status_Get(ADC_INT_FLAG_ENDC) != RESET)
    {
        /* Set PA6 pin */
        GPIO_Pins_Set(GPIOA, GPIO_PIN_6);

        ADCConvertedValue[0] = ADC_Regular_Group_Conversion_Data_Get(0);
        ADCConvertedValue[1] = ADC_Regular_Group_Conversion_Data_Get(1);
        /* Clear ADC ENDC pending interrupt bit */
        ADC_INTFlag_Status_Clear(ADC_INT_FLAG_ENDC);
        /* Reset PA6 pin */
        GPIO_Pins_Reset(GPIOA, GPIO_PIN_6);
    }
}
